...
# Load initial trial results
with open('experiment_results/initial_5crit_trial_1_20251125_152732.json', 'r') as f:
    initial_trial = json.load(f)
...
def parse_initial_results(trial_data):
    text_results = {}
    video_results = {}

    for entry in trial_data['text_based']:
        video = entry['video']
        criteria = entry['criteria']
        if video not in text_results:
            text_results[video] = {}

        # Extract result from the response
        if entry['success'] and entry['result']:
            match = re.search(rf'{criteria}\s*:\s*(\d+)', entry['result'])
            if match:
                text_results[video][criteria] = int(match.group(1))

    for entry in trial_data['video_based']:
        video = entry['video']
        criteria = entry['criteria']
        ...
        if entry['success'] and entry['result']:
            match = re.search(rf'{criteria}\s*:\s*(\d+)', entry['result'])
            if match:
                video_results[video][criteria] = int(match.group(1))

    return text_results, video_results

initial_text, initial_video = parse_initial_results(initial_trial)
...
print('='*80)
print('DID FEEDBACK CHANGE PREDICTIONS FOR THE 9 FEEDBACK CASES?')
print('='*80)
print()
...
for video, criteria in feedback_videos:
    ...
    # Initial predictions
    init_text = initial_text.get(video, {}).get(criteria, '?')
    init_video = initial_video.get(video, {}).get(criteria, '?')
    ...
    # Print details
    print(f'Video: {video[:55]}...')
    print(f'Criteria: {criteria} | Ground truth: {gt}')

    text_status = ""
    if text_fixed:
        text_status = "✅ FIXED"
    elif text_broke:
        text_status = "⚠️  BROKE (was correct, now wrong)"
    elif post_text != gt:
        text_status = "❌ STILL WRONG"
    elif text_changed:
        text_status = "✓ changed but both correct"
    else:
        text_status = "✓ unchanged (was already correct)"

    video_status = ""
    if video_fixed:
        video_status = "✅ FIXED"
    elif video_broke:
        video_status = "⚠️  BROKE (was correct, now wrong)"
    elif post_video != gt:
        video_status = "❌ STILL WRONG"
    elif video_changed:
        video_status = "✓ changed but both correct"
    else:
        video_status = "✓ unchanged (was already correct)"
